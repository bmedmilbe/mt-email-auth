...
class StringHelper():
    ...
    def simple_parent_text(self, parents):
        ...
        for person in parents.all():
            members[person.parent.degree] = members[person.parent.degree]+ [person] if members.get(person.parent.degree) else [person]
        ...
        for member in members.keys():
            ...
            if qtd == 1:
                his = f"seu {members[member][0].parent.title}, " if members[member][0].parent.gender == "M" else f"sua {members[member][0].parent.title}, "
            if qtd >= 2:
    ...
    def ext_days(self, dates:CertificateDate): 
        ...
        for month in dates.all():
            new_dates[month.date.month] = new_dates[month.date.month]+ [month] if new_dates.get(month.date.month) else [month]
        ...
        for month in new_dates.keys():
            ...
            if qtd >= 2:
                ...
                text = f"{text} de {self.calendar_month(int(new_dates[month][0].date.month))} de {new_dates[month][0].date.year}, "
