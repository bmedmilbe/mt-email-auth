...
class PDF
{
    ...
    public function __construct($text, $type1,AtestadoTypes $type2, Gerados $gerado, DocumentForm $form = null, Bis $bi = null)
    {
        $dompdf = new DOMPDF();
        //dnd($form);
        ...
        $HEAD = $this->head();
        //dnd($type2->getTypeId());
        if($type2->getTypeId() == 8 && ($bi->getNascLoc()->getCountryId() == 3 ||
                $bi->getNascLoc()->getCountryId() == 4 ||
                $bi->getNascLoc()->getCountryId() == 7 ||
                $bi->getNascLoc()->getCountryId() == 8 ||
                $bi->getNascLoc()->getCountryId() == 9 ||
                $bi->getNascLoc()->getCountryId() == 12 ||
                $bi->getNascLoc()->getCountryId() == 13 ||
                $bi->getNascLoc()->getCountryId() == 14 ||
                $bi->getNascLoc()->getCountryId() == 15
            ) 
            ){
            $conta = $this->conta($type1, $type2, $gerado->getAtestadoNumber(),1847.5,true);

        }elseif($type2->getTypeId() == 8){
            $conta = $this->conta($type1, $type2, $gerado->getAtestadoNumber(),2460,true);

        }elseif($type2->getTypeId() == 25){
            $conta = $this->conta($type1, $type2, $gerado->getAtestadoNumber(),
                $form->getModifics()->getModificValue());
        }elseif($type2->getTypeId() == 29){

            if($form->getMetros() == -1){
                $value = 250 * $form->getDias();
            }else{
                if($form->getMetros() >= 4){
                    $value = 175 * $form->getDias();
                }else{
                    $value = 175 * $form->getDias() + (intval($form->getMetros()) - 4) * 50 * $form->getDias();
                }
                $value = ($value) + 10;
            }

            $conta = $this->conta($type1, $type2, $gerado->getAtestadoNumber(), $value);
        }else{
            $conta = $this->conta($type1, $type2, $gerado->getAtestadoNumber());
        }
...
//$conta = "";
        ...
        $texto_final = $this->textoFinal($type1, $form);
...
";
    ...
    private function conta(DocumentTypes $type1, AtestadoTypes $types2, $atestado_number = 0, $autoV = 0, $cplp = false)
    {
        ...
        if($types2->getTypeId() == 25 || $types2->getTypeId() == 29 || ($types2->getTypeId() == 8 && $cplp = true)){
           $value = $autoV;
           $Total += $Rasa = 100;
        }else{
            $Total += $Rasa = 5;
        ...
        $string = $this->setTracoCenter(CONTA-3, $string);
        ?>
...
Américo da Conceição Fernandes de Ceita</p>
    ...
    {
        return "AMÉRICO DA CONCEIÇÃO FERNANDES DE CEITA, PRESIDENTE DA CÂMARA DISTRITAL DE MÉ-ZÓCHI.";
    }
